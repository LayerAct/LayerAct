...
from utils import train, validate, validate_10crop, validate_ImageNet_C
        ...
        for act_count, activation_name in enumerate(activation_name_list) : 
            ...
            if not duplicate and '{}_{}.csv'.format(corr, activation_name) in os.listdir(save_path) : 
                ...
                for trial in range(args.start_trial, args.end_trial+1) : 
                    ...
                    model = model_building(args, 1000)
                    model.to(device)
                    ...
                    test_loss, test_acc1, test_acc5 = validate_ImageNet_C(model, criterion, device, corr, path, args.batch_size, args.num_workers)
