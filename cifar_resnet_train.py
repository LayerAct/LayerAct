...
from data import image_data_building
...
from utils import cifar_training_args_building
...
if __name__ == '__main__' : 
    ...
    args = cifar_training_args_building(parser)
    print(args)
    ...
    for trial in range(args.start_trial, args.end_trial+1) : 
        ...
        if not duplicate and ('{}.pth.tar'.format(file_name) in os.listdir(pth_path) and '{}.npy'.format(file_name) in os.listdir(npy_path)) :
            ...
            if resume and os.path.isfile(checkpoint_path + file_name + '_checkpoint.pth.tar') : 
                ...
                checkpoint = torch.load(checkpoint_path + file_name + '_checkpoint.pth.tar', map_location=device)
            ...
            while iter < args.max_iter : 
                ...
                if is_best : 
                    ...
                    #print(round(test_acc1.item(), 3), end='\n')
            ...
            model = model.to(device)
            model.eval()
            model.load_state_dict(torch.load('{}.pth.tar'.format(pth_path + file_name)))
